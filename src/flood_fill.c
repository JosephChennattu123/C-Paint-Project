...
#include "util.h"
void jill(image_t *phill, int x1, int y1, pixel_t *target_color1,int wii,int hii,int rr,int gg,int bb)
{
    ...
    if((phill->img[(wii*y1)+x1].r == rr) && (phill->img[(wii*y1)+x1].g == gg) && (phill->img[(wii*y1)+x1].b == bb))
    {
        ...
        jill(phill, x1 + 1, y1, target_color1, wii, hii,rr,gg,bb);
        jill(phill, x1 - 1, y1, target_color1, wii, hii,rr,gg,bb);
        jill(phill, x1, y1 + 1, target_color1, wii, hii,rr,gg,bb);
        jill(phill, x1, y1 - 1, target_color1, wii, hii,rr,gg,bb);
    }
    return;
}
